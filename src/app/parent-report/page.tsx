  ...
  return (
    <div className="space-y-5">
      <section className="product-card p-4 sm:p-6">
        <p className="text-sm font-semibold text-[#9B111E]">부모 리포트</p>
        <h1 className="mt-1 text-2xl font-bold text-slate-950">오늘의 학습 상태</h1>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <MetricCard label="공부 시간" value={`${mockParentSummary.studyMinutes}분`} detail="오늘 누적 시간" tone="gold" />
          <MetricCard label="질문 수" value={`${mockParentSummary.questionCount}개`} detail="과목별 AI 라우팅 포함" tone="black" />
          <MetricCard label="복습 수" value={`${todayReviewCount}개`} detail="오늘 확인할 항목" tone="gold" />
          <MetricCard label="공부 과목" value={`${mockStudentProfile.targetSubjects.length}개`} detail={studySubjects} tone="ruby" />
        </div>
      </section>
      ...
      <section>
        <div className="product-card p-4 sm:p-5">
          <div className="flex items-center gap-2">
            <TrendingUp aria-hidden="true" size={19} className="text-[#9B111E]" />
            ...
            {mockParentSummary.trend.map((item) => (
              <div key={item.day} className="flex h-full flex-1 flex-col justify-end gap-2">
                <div className="flex flex-1 items-end">
                  <div
                    className="w-full rounded-t-lg bg-[#D4AF37]"
